...
class Model(object):
    def __init__(self, input_data, output_data, name, type, batch_size = 64):
        ...
        # why?
        self.output_data = np.reshape(output_data, (output_data.shape[0], output_data.shape[1]))
    ...
    def build_model(self, input_placeholder):
        x = tf.layers.dense(input_placeholder, 128, activation = tf.nn.tanh)
        x = tf.layers.dense(x, 64, activation = tf.nn.tanh)
        x = tf.layers.dense(x, self.output_shape[-1]) # again, figure out what does this mean
        return x
    ...
    def train(self, epochs = 20, train_data = None, test_data = None, number = None):
        ...
        for epoch in range(epochs):
            for idx in range(batch_idxs):
                # selecting data from block idx to idx+1
                batch_train = train_data[idx * self.batch_size : (idx+1) * self.batch_size]
                batch_vale = test_data[idx * self.batch_size : (idx+1) * self. batch_size]
                feed_train = {self.input_placeholder:batch_train, self.output_placeholder_true:batch_vale}
